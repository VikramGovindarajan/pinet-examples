...
def fun1(time,delt):
    if (time<=0):
        y = 10400
    if(0<time<=30):
        y = 10400-((1144/30)*(time))
    if(30<time<=300):
        y = 9256
    if (300<time<=550):
        y = 9256-((52/250)*(time-300))
    if (time>550):
        y = 9204
    return y
...
global series1
series1 = csv_reader("temp_PFS_450.csv")
def fun2(time,delt):
    y = series1(time)
    return y
...
global series2
series2 = csv_reader("power_PFS_450.csv")
def fun3(time,delt):
